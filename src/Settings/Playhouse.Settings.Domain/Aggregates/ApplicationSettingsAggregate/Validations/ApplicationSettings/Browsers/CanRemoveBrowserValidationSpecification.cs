...
{
    public sealed class CanRemoveBrowserValidationSpecification
    {
        private readonly IValidationSpecification<BrowserType> _validations;

        public CanRemoveBrowserValidationSpecification(IEnumerable<BrowserType> browsers)
        {
            _validations = new NotNullValidationSpecification()
                .Then(new NotMissingValidationSpecification(browsers));
        }
